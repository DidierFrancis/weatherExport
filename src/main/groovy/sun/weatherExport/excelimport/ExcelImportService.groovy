package sun.weatherExport.excelimport
...
import sun.weatherExport.plugins.excelimport.ExpectedPropertyType
import sun.weatherExport.plugins.excelimport.ImportCellCollector
import sun.weatherExport.plugins.excelimport.NoopImportCellCollector
...
class ExcelImportService {
    ...
    Map createColumnMapFromColumnNames(List COLUMN_NAMES, Row headerRow) {
        ...
        for (Cell c : headerRow) {
            if (COLUMN_NAMES.contains(c.stringCellValue)) {
                println "Found index for column ${c.stringCellValue}"
                columnIndexMap[c.stringCellValue] = c.columnIndex
            }
        }

        println "columnIndexMap $columnIndexMap"
        columnIndexMap
    }
    ...
    def convertColumnMapOneRow(Sheet currentSheet, Map config, int rowIndex, ImportCellCollector pcc = null, FormulaEvaluator evaluator = null, propertyConfigurationMap = null) {
        ...
        if (!row) {
            println "Row is null at row $rowIndex, sheet ${currentSheet.sheetName}"
            return returnParams
        }
        ...
        config.columnMap.each { columnName, propertyName ->
            try {
                ...
                println "\t\tValue for $propertyName (column ${columnName}) is $value "
                if (value == null || ''.equals(value)) { //cheking for null, because otherwise 0 value will fail here
                    println "Value for column $columnName row $rowIndex is null or empty String.  Was trying to find property $propertyName.  Skipping setting its value in param map"
        ...
        println "Returning $returnParams for Row $rowIndex, columnMap $config.columnMap"
    ...
    void convertOneRowToColumnMap(inputParams, Sheet currentSheet, Map columnMap, int rowIndex, FormulaEvaluator evaluator = null, propertyConfigurationMap = null) {
        ...
        columnMap.each { columnName, propertyName ->
            try {
                ...
                println "Exception caught at row $rowIndex column $columnName while trying to get property $propertyName", e
                //continue in the loop, so can collect other properties
            }
        }
    }
    ...
    def convertFromCellMapToMapWithValues(Sheet currentSheet, Map cellMap, ImportCellCollector pcc = null, FormulaEvaluator evaluator = null, Map propertyConfigurationMap = [:]) {
        ...
        cellMap.each { String cellName, String propertyName ->
            try {
                ...
                //fix by Jackson
                if (value == null) {
                    println "No value in cell $cellName set.  Was trying to find $propertyName"
                } else {
        ...
        println "Returning objectParams $objectParams"
        objectParams
    }
    ...
    Serializable getCellValueByColName(Row row, String columnName, ImportCellCollector pcc = null, FormulaEvaluator evaluator = null, Map propertyConfiguration = [:]) {
        ...
        println "getCellValueByColName $columnName row ${row.rowNum}, propConfig $propertyConfiguration, colIndex = $colIndex"
            ...
            case CellType.STRING:
                ...
                if (!propertyConfiguration || propertyConfiguration.expectedType == StringType) {
                    ...
                    if (propertyConfiguration && strValue == propertyConfiguration?.valueEquivalentToNull) {
                        println("Found a value that's not null (value ${strValue}), but configuration property says to return null anyway")
                        return null
                    }
                    //println "Encountered unexpected string cell Value ${cell.stringCellValue} at row ${cell.getRowIndex()} column ${cell.getColumnIndex()}"
                    return strValue
                }
                ...
                if (propertyConfiguration?.expectedType == IntType) {
                    println "${getCellAddresString(cell)} Expected Type is INT, but cell type is String, trying to extract numeric value"
                    try {
                        return cell.stringCellValue?.toInteger()
                    } catch (Exception e) {
                        println "${getCellAddresString(cell)} Cannot get numeric value, returning default value specified for this type, which is ${propertyConfiguration.defaultValue}"
                        pcc.reportCell(cell, propertyConfiguration)
                        return propertyConfiguration.defaultValue
                    }
                }
                ...
                if (propertyConfiguration?.expectedType == DoubleType) {
                    println "${getCellAddresString(cell)}Expected Type is DOUBLE, but cell type is String, trying to extract numeric value"
                    try {
                        return cell.stringCellValue?.toDouble()
                    } catch (Exception e) {
                        println "${getCellAddresString(cell)} Cannot get double value, returning default value specified for this type", e
                        pcc.reportCell(cell, propertyConfiguration)
                        return propertyConfiguration.defaultValue
                    }
                }
                ...
                if (propertyConfiguration?.expectedType == EmailType) {
                    ...
                    def emailValidator = EmailValidator.newInstance()
                    if (emailValidator.isValid(strValue)) {
                        return strValue
                    }
                    println "${getCellAddresString(cell)} Cannot get email value, returning default value specified for this type, which is ${propertyConfiguration.defaultValue}"
                    pcc.reportCell(cell, propertyConfiguration)
                    return propertyConfiguration.defaultValue
                }

                println "${getCellAddresString(cell)} Potential issue -  ${getCellAddresString(cell)}the excel file claims the type is String, but expecting something else for cell with value $origcell. Returning default value of ${propertyConfiguration?.defaultValue}"
...
*/
                ...
                break
            case CellType.ERROR:
                println "CELL Type is ERROR value: $cell.errorCellValue ${getCellAddresString(cell)}"
                pcc.reportCell(cell, propertyConfiguration)
                return null
            case CellType.FORMULA:
                println "Cell type is formula, returning null  ${getCellAddresString(cell)}"
                pcc.reportCell(cell, propertyConfiguration)
                return null
            case CellType.BOOLEAN:
                return cell.booleanCellValue
            case CellType.BLANK:
                println "Found blank cell at ${getCellAddresString(cell)}"
                pcc.reportCell(cell, propertyConfiguration)
                return null
            default:
                println "Unexpected cell type.  ${getCellAddresString(cell)} Ignoring.  Cell Value [${cell}] type ${cell.cellTypeEnum}"
        }
        println "WARNING: RETURNING NULL FROM getCellValue.  UNEXPECTED CONDITION ${getCellAddresString(cell)}"
        pcc.reportCell(cell, propertyConfiguration)
        return null
    }
    ...
    void setCellValue(value, Cell origcell, FormulaEvaluator evaluator = null, propertyConfiguration = [:]) {
        ...
        switch (propertyConfiguration.expectedType) {
            ...
            case ExpectedPropertyType.DoubleType:
                ...
                println "value $value was supposed tobe numeric but isn't"
                //... type convertion handling
                break
            ...
            case ExpectedPropertyType.DateJavaType:
                ...
                break
            default:
                println "Unexpected property type.  Ignoring.  Property Value [${value}] type ${propertyConfiguration.expectedType}"
        }
        println "WARNING: RETURNING FROM setCellValue.  UNEXPECTED CONDITION expectedType ${propertyConfiguration.expectedType} / ${value}"
        return
    }
