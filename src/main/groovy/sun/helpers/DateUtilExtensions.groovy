...
public class DateUtilExtensions {
     ...
     * @see java.util.Calendar* @since 1.5.5
     */
     ...
     * @param field a Calendar field, e.g. MONTH
     * @return the value for the given field, e.g. FEBRUARY
     * @see java.util.Calendar* @since 1.7.3
     */
     ...
     * @see java.text.SimpleDateFormat* @since 1.5.7
     */
     ...
     * @return a string representation of this date.
     * @see java.text.SimpleDateFormat* @since 1.8.3
     */
    ...
    public static LocalDateTime setTimeZone(Date self, TimeZone tz) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
        sdf.setTimeZone(tz);
        def str = sdf.format(self);
    ...
    static def getDecades(Calendar to) {
        Calendar self = Calendar.getInstance()
        self.set((self.get(Calendar.YEAR) - 1), Calendar.JANUARY, 1)
        def list = []
        int cpt = 0

        for (Date i = (Date) self.time; i.compareTo(to.time) <= 0; i = plus(i, 10)) {
            // log.info("date ${i}")
            cpt++
            def dateDebut = format(i, "dd/MM/yyyy")
            def dateFin = format(plus(i, 10), "dd/MM/yyyy")
            list << [index: cpt, date: "du ${dateDebut} au ${dateFin}"]

        }
        return list
    }

    def static listPattern() {
        String[] list = [
                "yy-MM-dd",
                "y-M-d",
                "d-M-y",
    ...
    static def getDateAtNextHour(Date date) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
    ...
    static def getDateAtLastHour(Date date) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
