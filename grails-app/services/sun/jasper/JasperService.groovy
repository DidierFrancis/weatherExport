...
import sun.plugins.jasper.*
import sun.plugins.jasper.JasperExportFormat
import sun.plugins.jasper.JasperReportDef
...
class JasperService {
    ...
    /**
     * Generate a report based on a single sun.jasper file.
     * @param format , target format
     * @param reportDef , sun.jasper report object
    ...
    /**
     * Generate a single report based on a list of sun.jasper files.
     * @param format , target format
    ...
    /**
     * Forces the Jasper Reports  temp folder to be "~/.grails/.sun.jasper" and ensures that such a folder exists.
    ...
    protected void forceTempFolder() {
        ...
        if (FORCE_TEMP_FOLDER) {
            ...
            File tempFolder = new File(userHomeDir, "/.grails/.sun.jasper")

            // This is the current official means for setting the temp folder for sun.jasper reports to use when compiling
            ...
            System.setProperty("sun.jasper.reports.compile.temp", tempFolder.getAbsolutePath())
    ...
    private JasperPrint generatePrinter(JasperReportDef reportDef) {
        ...
        if (jrDataSource != null) {
            if (resource.getFilename().endsWith('.sun.jasper')) {
